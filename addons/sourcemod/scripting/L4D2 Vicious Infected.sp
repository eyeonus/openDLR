...
{
	CreateConVar("l4d_vts_version", PLUGIN_VERSION, "Vicious Infected Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	...
	cvarCharger = CreateConVar("l4d_vts_charger", "1", "Enables the Charger abilities. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBrokenRibs = CreateConVar("l4d_vts_brokenribs", "1", "After a pummel ends the Survivor takes damage over time. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBrokenRibsChance = CreateConVar("l4d_vts_brokenribschance", "100", "Chance that after a pummel ends the Survivor takes damage over time (100 = 100%). (Def 100)", FCVAR_NOTIFY, true, 1.0, false, _);
	cvarBrokenRibsDuration = CreateConVar("l4d_vts_brokenribsduration", "10", "For how many seconds does the Broken Ribs last. (Def 10)", FCVAR_NOTIFY, true, 1.0, false, _);
	CreateConVar("l4d_vts_brokenribsdamage", "1", "How much damage is inflicted by Broken Ribs each second. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSnappedLeg = CreateConVar("l4d_vts_snappedleg", "1", "After a charger collision movement speed is reduced. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSnappedLegChance = CreateConVar("l4d_vts_snappedlegchance", "100", "Chance that after a charger collision movement speed is reduced. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSnappedLegDuration = CreateConVar("l4d_vts_snappedlegduration", "5", "For how many seconds will the Snapped Leg reduce movement speed (100 = 100%). (Def 100)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSnappedLegSpeed = CreateConVar("l4d_vts_snappedlegspeed", "0.5", "How much does Snapped Leg reduce movement speed. (Def 0.5)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarStowaway = CreateConVar("l4d_vts_stowaway", "1", "After a Charge the Survivor is damaged based upon carry time. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarStowawayDamage = CreateConVar("l4d_vts_stowawaydamage", "5", "How much damage is inflicted by Stowaway for each second carried. (Def 5)", FCVAR_NOTIFY, true, 0.0, false, _);
	...
	cvarHunter = CreateConVar("l4d_vts_hunter", "1", "Enables the Hunter abilities. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarCobraStrike = CreateConVar("l4d_vts_cobrastrike", "0", "Hunter pounce has a chance of instantly incapping and making Survivor B&W. (Def 0)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarCobraStrikeChance = CreateConVar("l4d_vts_cobrastrikechance", "5", "Chance that the Hunter pounce will incap Survivor (5 = 5%). (Def 5)", FCVAR_NOTIFY, true, 0.0, false, _);
	CreateConVar("l4d_vts_cobrastrikedamage", "100", "How much damage is inflicted by Cobra Strike (should be max Survivor HP). (Def 100)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarDeepWounds = CreateConVar("l4d_vts_deepwounds", "1", "After a pounce ends the Survivor takes damage over time. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarDeepWoundsChance = CreateConVar("l4d_vts_deepwoundschance", "100", "Chance that after a pounce ends the Survivor takes damage over time (100 = 100%). (Def 100)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarDeepWoundsDuration = CreateConVar("l4d_vts_deepwoundsduration", "10", "For how many seconds does the Deep Wounds last. (Def 10)", FCVAR_NOTIFY, true, 1.0, false, _);
	CreateConVar("l4d_vts_deepwoundsdamage", "1", "How much damage is inflicted by Deep Wounds each second. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	...
	cvarTank= CreateConVar("l4d_vts_tank", "1", "Enables the Tank abilities. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarRoidRage= CreateConVar("l4d_vts_roidrage", "1", "Enables randomizing the Tank HP. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarRoidRageMin= CreateConVar("l4d_vts_roidragemin", "5000", "Minimum HP for the Tank. (Def 5000)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarRoidRageMax= CreateConVar("l4d_vts_roidragemax", "9000", "Maximum HP for the Tank. (Def 9000)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTankMax = CreateConVar("l4d_vts_tankmax", "2", "Maximum number of Tanks (Mini Bodyguard) allowed to spawn with this plugin at a time. (Def 2)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTankMaxMap = CreateConVar("l4d_vts_tankmaxmap", "4", "Maximum number of Tanks (Mini Bodyguard) allowed to spawn throughout an entire map. (Def 4)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBurningRage = CreateConVar("l4d_vts_burningrage", "1", "Increases the Tanks speed when on fire. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBurningRageSpeed = CreateConVar("l4d_vts_burningragespeed", "1.25", "How much of a speed boost does Burning Rage give. (Def 1.25)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarCullingSwarm = CreateConVar("l4d_vts_cullingswarm", "1", "Spawns a Panic Event when the tank spawns. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarMourningWidow = CreateConVar("l4d_vts_mourningwidow", "1", "Spawns a Witch after the Tank dies. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarWorriedWife = CreateConVar("l4d_vts_worriedwife", "0", "Spawns a Witch when the Tank spawns. (Def 0)", FCVAR_NOTIFY, true, 0.0, false, _);
	...
	cvarWitch = CreateConVar("l4d_vts_witch", "1", "Enables the Witch abilities. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarMoodSwing= CreateConVar("l4d_vts_moodswing", "1", "Enables randomizing the Witch HP. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarMoodSwingMin= CreateConVar("l4d_vts_moodswingmin", "1000", "Minimum HP for the Witch. (Def 1000)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarMoodSwingMax= CreateConVar("l4d_vts_moodswingmax", "2000", "Maximum HP for the Witch. (Def 2000)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarWitchMax = CreateConVar("l4d_vts_witchmax", "2", "Maximum number of Witches allowed to spawn with this plugin at a time. (Def 2)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarWitchMaxMap = CreateConVar("l4d_vts_witchmaxmap", "6", "Maximum number of Witches allowed to spawn throughout an entire map. (Def 6)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarMiniBodyguard = CreateConVar("l4d_vts_minibodyguard", "1", "Spawns a Tank when the Witch is killed. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarMiniBodyguardHP = CreateConVar("l4d_vts_minibodyguardhp", "0.25", "Percentage of HP to give the Bodyguard (0.25 = 25%). (Def 0.25)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarMiniHealth = CreateConVar("l4d_vts_minihealth", "1", "Changes the health of the Mini Bodyguard Tank. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSupportGroup = CreateConVar("l4d_vts_supportgroup", "1", "Spawns a mob when the Witch is angered. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
