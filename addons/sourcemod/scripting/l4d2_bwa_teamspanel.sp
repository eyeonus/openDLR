...
Unscramble() {
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{	
			...
			GetClientAuthId(i, AuthId_Engine, steamId, STEAMID_LENGTH);
...
public OnClientDisconnect(client) {
	...
	GetClientAuthId(client, AuthId_Engine, steamId, STEAMID_LENGTH);
...
public Action:Timer_MoveToSpec(Handle:timer, any:client) {
	...
	GetClientAuthId(client, AuthId_Engine, auth, STEAMID_LENGTH);
		...
		if (IsClientConnected(i) && IsClientInGame(i))
		{
			...
			if ( (team == TEAM_SPECTATOR) || (team == TEAM_SURVIVOR) || (team == TEAM_INFECTED) )
			{
				...
				else
				{
					...
					GetClientAuthId(i, AuthId_Engine, PlayerSteamID[i], STEAMID_LENGTH);
				}				
			}
		}
	}	
...
PerformSwitch(client, target, team, bool:silent) {
	...
	if (team == TEAM_SURVIVOR)
	{
		...
		while (!(IsClientConnected(bot) && IsFakeClient(bot) && (GetClientTeam(bot) == TEAM_SURVIVOR))) { bot++; }
